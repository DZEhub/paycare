...
* The workflow should:
  ...
  * Volumes are mounted correctly so the output CSV file is written to a known location.
  ...
  1. **Fail the workflow** if tests fail or if the container exits with a non-zero status code.
  2. (Optional) **Upload artifacts** such as the processed CSV or test reports using `actions/upload-artifact`.
...
* [https://github.com/JedhaBootcamp/paycare](https://github.com/JedhaBootcamp/paycare)
